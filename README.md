# 👋 Hi, I'm Hanuman Sahani!

## About Me
I am a **Flutter & Laravel Developer** passionate about building high-performance mobile applications and scalable backend solutions. With expertise in **Dart, PHP, Laravel, and Flutter**, I focus on creating seamless user experiences and robust API architectures.

I love solving complex problems, optimizing performance, and contributing to open-source projects. My goal is to build intuitive and scalable applications that make an impact. 🚀

## 🚀 Skills & Technologies
- **Frontend:** Flutter (Dart), Provider, Riverpod, Bloc/Cubit, Clean Architecture
- **Backend:** Laravel (PHP), REST APIs, Sanctum/JWT Authentication, Laravel Queues
- **Database:** MySQL, PostgreSQL, Firebase Firestore
- **Tools & DevOps:** Git, Docker, Firebase, AWS, CI/CD, Postman
- **Other Technologies:** Google ML Kit (Face Recognition), WebSockets, Payment Integrations, GraphQL
- **Testing & Debugging:** Unit Testing, Integration Testing, Debugging with Flutter DevTools

## 🌟 What I'm Working On
- 📱 Developing feature-rich mobile applications with **Flutter**
- 🔧 Optimizing and scaling Laravel-based backends
- 🚀 Exploring AI-powered **face recognition** for attendance systems
- 🛠️ Implementing robust **state management** techniques
- 🌍 Open-source contributions & tech blogging
- 📊 Data visualization & analytics with Flutter

## 🎯 Achievements & Projects
- 🏆 Built a **face recognition-based attendance system** using Flutter & Laravel
- 📌 Developed an **employee trip management system** for real-time route tracking
- 🔥 Contributed to open-source projects, improving API performance & security
- 📈 Designed a **scalable backend architecture** handling thousands of users
- 💼 Developed a **Human Resource Management System (HRMS)** for employee management  
  🔗 [View Project](https://payroll.daytek.in)
- 💰 Built a **Payroll System** for automated salary processing and tax calculations  
  🔗 [View Project](https://payroll.daytek.in)
- 🛒 Created an **E-commerce platform** with a robust shopping cart and payment gateway  
  🔗 [View Project](https://play.google.com/store/apps/details?id=com.utsavfashion.app)
- 🏢 Designed and deployed a **complete ERP solution** for business operations  
  🔗 [View Project](https://play.google.com/store/apps/details?id=com.akcworld.stm)
- 📍 Developed a **Business Listing Platform** to connect businesses with customers  
  🔗 [View Project](https://play.google.com/store/apps/details?id=com.SrankBazaar.BusinessListing)

## 📫 Connect With Me
- GitHub: [github.com/yourusername](https://github.com/king7388)
- LinkedIn: [linkedin.com/in/yourusername](https://www.linkedin.com/in/hanuman-sahani-928404246?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app)
- Email: hanumansahani7388@gmail.com

## 📜 GitHub Stats

![Your GitHub Stats](https://github-readme-stats.vercel.app/api?username=king7388&show_icons=true&count_private=true&hide_title=true&hide=prs,issues&theme=radical&bg_color=00000000&card_width=500)

### 🚀 Top Languages

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=king7388&layout=compact&theme=radical&langs_count=10)

### 📈 Contribution Streak

![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=king7388&theme=radical)


## 💡 Let's Collaborate!
I am always open to exciting projects, collaborations, and knowledge sharing. If you have an idea, let's build something amazing together! 🚀

